...
void MainWindow::posDetect(const Mat &in, vector<RotatedRect> & rects) //寻找区域
...
//	Mat threshold_Inv = imgInv - threshold_R; //黑白色反转，即背景为黑色
	...
	Mat out, roi_img;
	   ...
	   imshow("NUMBER", out);
		...
		QString aa = QString(QLatin1String(outt));
		ui->lineEdit_5->setText(aa);
	   */
}
...
{
	float error = 0.2;
	const float aspect = 4.5 / 0.3; //长宽比  4.5 / 0.3
	int min = 10 * aspect * 10; //最小区域 10 * aspect * 10
	int max = 50 * aspect * 50;  //最大区域 50 * aspect * 50
...
{
	float error = 0.25;
	const float aspect = 80 / 25; //长宽比
	int min = 20 * aspect * 20; //最小区域
	int max = 80 * aspect * 80;  //最大区域
	float rmin = aspect - aspect*error; //考虑误差后的最小长宽比
	float rmax = aspect + aspect*error; //考虑误差后的最大长宽比
...
{
	float error = 0.2;
	const float aspect = 1.0 / 1; //长宽比  4.5 / 0.3
	int min = 15 * aspect * 15; //最小区域 10 * aspect * 10
	int max = 30 * aspect * 30;  //最大区域 50 * aspect * 50
	float rmin = aspect - aspect*error; //考虑误差后的最小长宽比
	float rmax = aspect + aspect*error; //考虑误差后的最大长宽比
...
void MainWindow::find_card_area(const Mat &in, vector<RotatedRect> & rects) //判断并截取身份证区域
{
	...
	{
		RotatedRect mr = minAreaRect(Mat(*itc)); //返回每个轮廓的最小有界矩形区域
		if (!is_card_area(mr))  //判断矩形轮廓是否符合要求
		{
			itc = contours.erase(itc);
...
{
	float error = 0.2;
	const float aspect = 856 / 540; //长宽比  4.5 / 0.3
	int min = 350 * aspect * 350; //最小区域 10 * aspect * 10
	int max = 800 * aspect * 800;  //最大区域 50 * aspect * 50
	float rmin = aspect - aspect*error; //考虑误差后的最小长宽比
	float rmax = aspect + aspect*error; //考虑误差后的最大长宽比
...
Mat MainWindow::resize_img(Mat &src)
{
	cv::Size2i size = Size(856, 540);
	Mat dst;
